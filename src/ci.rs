...
fn render_markdown(
    ...
    // ── Title ─────────────────────────────────────────────────────────────
    let pg_version = reports.first().map(|r| r.pg_version).unwrap_or(14);
    md.push_str(&format!(
        "## 🔍 SchemaRisk — Migration Safety Report  *(PostgreSQL {})*\n\n",
        pg_version
    ));
    ...
    md.push_str(
        "---\n\
         <sub>\n\
         🛡️ Generated by <strong><a href=\"https://github.com/Keystones-Lab/Schema-risk\">SchemaRisk</a></strong> \
         — Prevent dangerous database migrations before they reach production.\n\n\
         Detects lock-heavy operations • Generates safe migration plans • Scans codebase for blast radius\n\
         </sub>\n",
    );
...
fn render_file_section(
    ...
    // ── PG version notice ─────────────────────────────────────────────────
    md.push_str(&format!(
        "<sub>Analyzed against PostgreSQL {} rules. \
         Use `--pg-version` to change target version.</sub>\n\n",
        r.pg_version
    ));
...
mod tests {
    ...
    fn test_github_comment_contains_table_header() {
        ...
        assert!(output.contains("SchemaRisk — Migration Safety Report"));
        assert!(output.contains("PostgreSQL 14"));
